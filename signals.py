...
def _compute_scalp_reversal_stabilization(
    ...
    *,
    ...
    """Small reversal-specific tape helper for SCALP PRE assistance.

    Looks for post-flush stabilization / reclaim lean without demanding the same
    continuation-style pressure required by RIDE breakout logic.
    """
    out: Dict[str, float | bool | None] = {
        "stabilizing": False,
        "flush_present": False,
    ...
    try:
        ...
        close_recent = pd.to_numeric(recent['close'], errors='coerce').dropna()
        open_recent = pd.to_numeric(recent['open'], errors='coerce').dropna()
        low_recent = pd.to_numeric(recent['low'], errors='coerce').dropna()
        high_recent = pd.to_numeric(recent['high'], errors='coerce').dropna()
        close_prior = pd.to_numeric(prior['close'], errors='coerce').dropna()
        open_prior = pd.to_numeric(prior['open'], errors='coerce').dropna()
        low_prior = pd.to_numeric(prior['low'], errors='coerce').dropna()
        high_prior = pd.to_numeric(prior['high'], errors='coerce').dropna()
        macd_tail = pd.to_numeric(df.get('macd_hist', pd.Series(index=df.index, dtype=float)).tail(4), errors='coerce').dropna()
        if min(len(close_recent), len(open_recent), len(low_recent), len(high_recent), len(close_prior), len(open_prior), len(low_prior), len(high_prior)) < 3:
            return out

        if str(direction).upper() == 'LONG':
            prior_push = float((open_prior.iloc[0] - low_prior.min()) / max(1e-9, atr_val))
            flush_present = bool(prior_push >= 0.55 or (close_prior.iloc[-1] < open_prior.iloc[0] - 0.35 * atr_val))
            lows_stable = bool(low_recent.min() >= low_prior.min() - 0.10 * atr_val)
            closes_improving = bool(close_recent.iloc[-1] >= close_recent.iloc[0] - 0.05 * atr_val and close_recent.mean() >= low_recent.min() + 0.54 * max(1e-9, float(high_recent.max() - low_recent.min())))
            reclaim_hold = bool(close_recent.iloc[-1] >= ref - 0.28 * atr_val)
            macd_improving = bool(len(macd_tail) >= 3 and macd_tail.iloc[-1] >= macd_tail.iloc[-2] >= macd_tail.iloc[-3])
        else:
            prior_push = float((high_prior.max() - open_prior.iloc[0]) / max(1e-9, atr_val))
            flush_present = bool(prior_push >= 0.55 or (close_prior.iloc[-1] > open_prior.iloc[0] + 0.35 * atr_val))
            lows_stable = bool(high_recent.max() <= high_prior.max() + 0.10 * atr_val)
            closes_improving = bool(close_recent.iloc[-1] <= close_recent.iloc[0] + 0.05 * atr_val and close_recent.mean() <= high_recent.max() - 0.54 * max(1e-9, float(high_recent.max() - low_recent.min())))
            reclaim_hold = bool(close_recent.iloc[-1] <= ref + 0.28 * atr_val)
            macd_improving = bool(len(macd_tail) >= 3 and macd_tail.iloc[-1] <= macd_tail.iloc[-2] <= macd_tail.iloc[-3])
        ...
        out.update({
            ...
            'reclaim_hold': bool(reclaim_hold),
            'bonus': float(bonus),
        })
        return out
    except Exception:
    ...
    *,
    ...
    try:
        ...
        if str(direction).upper() == 'LONG':
            ...
            wick_ratio = float((upper_wick / rng).replace([np.inf, -np.inf], np.nan).fillna(0.0).mean())
            close_finish = float(((c - l) / rng).replace([np.inf, -np.inf], np.nan).fillna(0.5).mean())
            touches = bool((h >= rel - 0.08 * atr_val).sum() >= 2)
            stuffing = bool(touches and wick_ratio >= 0.33 and close_finish <= 0.58)
            penalty = 0.0
            if stuffing:
                penalty += 1.0
            elif wick_ratio >= 0.28 and close_finish <= 0.62:
                penalty += 0.5
        else:
            lower_wick = (pd.concat([o, c], axis=1).min(axis=1) - l).clip(lower=0.0)
            wick_ratio = float((lower_wick / rng).replace([np.inf, -np.inf], np.nan).fillna(0.0).mean())
            close_finish = float(((h - c) / rng).replace([np.inf, -np.inf], np.nan).fillna(0.5).mean())
            touches = bool((l <= rel + 0.08 * atr_val).sum() >= 2)
            stuffing = bool(touches and wick_ratio >= 0.33 and close_finish <= 0.58)
            penalty = 0.0
            if stuffing:
                penalty += 1.0
        ...
        out.update({
            ...
            'close_finish': float(close_finish),
        })
        return out
    except Exception:
        return out
...
def _compute_breakout_urgency(
    *,
    tape_metrics: Dict[str, float | bool | None],
    near_break: bool,
    dist_br: float,
    dist_pb_band: float,
    prox_dist: float,
    hold_progress: bool,
    shallow_retests: bool,
    breakout_acceptance_ok: bool,
    ...
    try:
        ...
        if near_break:
            urgency += 1.0
        if float(tape_metrics.get('release_proximity') or 0.0) >= 1.5:
            urgency += 1.0
        if float(tape_metrics.get('pressure') or 0.0) >= 1.5 and bool(tape_metrics.get('macd_directional_build') or False):
            urgency += 1.0
        if shallow_retests and hold_progress and breakout_acceptance_ok:
            urgency += 1.0
        if float(dist_br) <= min(float(dist_pb_band), 0.75 * float(prox_dist)):
            urgency += 1.0
        urgency -= float(rejection_penalty or 0.0)
        return float(max(0.0, urgency))
    except Exception:
        return 0.0
...
def compute_scalp_signal(
    ...
    *,
    ...
    scalp_stab_long = {"stabilizing": False, "flush_present": False, "cluster_improving": False, "reclaim_hold": False, "bonus": 0.0}
    scalp_stab_short = {"stabilizing": False, "flush_present": False, "cluster_improving": False, "reclaim_hold": False, "bonus": 0.0}
    tape_long_bonus = 0
    tape_short_bonus = 0
    tape_pre_long_assist = False
    tape_pre_short_assist = False
    tape_long_effective_readiness = 0.0
    tape_short_effective_readiness = 0.0
    if tape_mode_enabled:
        long_baseline_ok = bool(pre_trigger_near and reclaim_attempt_long and (struct_pre_long or liquidity_event or orb_event) and ((macd_build_long_pts >= 3) or macd_pre_long or rsi_pre_long))
        short_baseline_ok = bool(pre_trigger_near and reclaim_attempt_short and (struct_pre_short or liquidity_event or orb_event) and ((macd_build_short_pts >= 3) or macd_pre_short or rsi_pre_short))
        ...
        tape_short = _compute_tape_readiness(
            ...
            structural_level=float(ref_vwap) if isinstance(ref_vwap, (float, int)) else None,
            trigger_near=bool(pre_trigger_near),
            baseline_ok=short_baseline_ok,
        )
        scalp_stab_long = _compute_scalp_reversal_stabilization(
            df, direction="LONG", ref_level=float(ref_vwap) if isinstance(ref_vwap, (float, int)) else None, atr_last=float(atr_last) if atr_last is not None else None,
        )
        scalp_stab_short = _compute_scalp_reversal_stabilization(
            df, direction="SHORT", ref_level=float(ref_vwap) if isinstance(ref_vwap, (float, int)) else None, atr_last=float(atr_last) if atr_last is not None else None,
        )
        tape_long_effective_readiness = float(tape_long.get("readiness") or 0.0)
        if long_baseline_ok and reclaim_attempt_long and bool(scalp_stab_long.get("stabilizing")):
            tape_long_effective_readiness += float(min(1.0, scalp_stab_long.get("bonus") or 0.0))
            if float(tape_long.get("pressure") or 0.0) >= 0.75 and bool(scalp_stab_long.get("reclaim_hold")):
                tape_long_effective_readiness += 0.35
        tape_short_effective_readiness = float(tape_short.get("readiness") or 0.0)
        if short_baseline_ok and reclaim_attempt_short and bool(scalp_stab_short.get("stabilizing")):
            tape_short_effective_readiness += float(min(1.0, scalp_stab_short.get("bonus") or 0.0))
            if float(tape_short.get("pressure") or 0.0) >= 0.75 and bool(scalp_stab_short.get("reclaim_hold")):
                tape_short_effective_readiness += 0.35
        tape_long_effective_readiness = float(min(8.0, tape_long_effective_readiness))
        tape_short_effective_readiness = float(min(8.0, tape_short_effective_readiness))

        tape_long_bonus = _tape_bonus_from_readiness(
            tape_long_effective_readiness,
            cap=3,
            thresholds=(5.0, 6.25, 7.0, 8.0),
        )
        tape_short_bonus = _tape_bonus_from_readiness(
            tape_short_effective_readiness,
            cap=3,
            thresholds=(5.0, 6.25, 7.0, 8.0),
        )
        if tape_long_bonus:
        ...
        tape_pre_long_assist = bool(
            tape_long_effective_readiness >= 6.0
            and pre_trigger_near
            and reclaim_attempt_long
            and ((macd_build_long_pts >= 3) or macd_pre_long or rsi_pre_long or bool(scalp_stab_long.get("cluster_improving")))
            and (struct_pre_long or liquidity_event or orb_event or bool(scalp_stab_long.get("stabilizing")))
            and (confirmation_score >= max(1, confirm_threshold - 2))
            and (not vwap_event)
        )
        tape_pre_short_assist = bool(
            tape_short_effective_readiness >= 6.0
            and pre_trigger_near
            and reclaim_attempt_short
            and ((macd_build_short_pts >= 3) or macd_pre_short or rsi_pre_short or bool(scalp_stab_short.get("cluster_improving")))
            and (struct_pre_short or liquidity_event or orb_event or bool(scalp_stab_short.get("stabilizing")))
            and (confirmation_score >= max(1, confirm_threshold - 2))
            and (not vwap_event)
        )
    ...
    extras["tape_effective_readiness_long"] = float(tape_long_effective_readiness or 0.0)
    extras["tape_effective_readiness_short"] = float(tape_short_effective_readiness or 0.0)
    ...
    extras["tape_reversal_stabilization_long"] = bool(scalp_stab_long.get("stabilizing"))
    extras["tape_reversal_stabilization_short"] = bool(scalp_stab_short.get("stabilizing"))
    extras["tape_reversal_bonus_long"] = float(scalp_stab_long.get("bonus") or 0.0)
    extras["tape_reversal_bonus_short"] = float(scalp_stab_short.get("bonus") or 0.0)
    ...
    # blend the reclaim/rejection anchor with nearby structure so entry logic is not
    # tied too rigidly to a single raw VWAP print when the market is already proving
    # acceptance around a slightly different structural level.
    ...
    try:
        if bias == "LONG":
            ...
            if isinstance(ref_vwap, (float, int)):
                anchors.append(float(ref_vwap)); weights.append(0.58 + 0.10 * hold_count)
            if isinstance(pivot_anchor, (float, int)):
                ...
                weights.append(0.22 + 0.18 * float(micro_hl) + 0.10 * pivot_near)
            ...
            if isinstance(ema20_ref, (float, int)):
                ...
                anchors.append(float(ema20_ref)); weights.append(0.12 + 0.08 * float(trend_long_ok) + 0.05 * ema_near)
            ...
            if isinstance(pivot_anchor, (float, int)):
                pivot_near = max(0.0, 1.0 - (abs(float(last_price) - float(pivot_anchor)) / max(1e-9, 1.25 * float(atr_last or 1.0))))
                anchors.append(float(pivot_anchor)); weights.append(0.22 + 0.18 * float(micro_lh) + 0.10 * pivot_near)
            if isinstance(ema20_ref, (float, int)):
                ema_near = max(0.0, 1.0 - (abs(float(last_price) - float(ema20_ref)) / max(1e-9, 1.50 * float(atr_last or 1.0))))
                anchors.append(float(ema20_ref)); weights.append(0.12 + 0.08 * float(trend_short_ok) + 0.05 * ema_near)
            if anchors and sum(weights) > 0:
                scalp_accept_line = float(np.average(np.asarray(anchors, dtype=float), weights=np.asarray(weights, dtype=float)))
...
def compute_ride_signal(
    ...
    **_ignored: object,
    ...
    # weight the eligible structural anchors by source confidence and hold quality,
    # rather than blindly locking to a single strongest raw source.
    ...
    if direction == "LONG":
        if ref_vwap is not None and np.isfinite(ref_vwap):
            ...
            accept_component_weights["VWAP"] = max(0.05, 0.30 + 0.55 * float(np.clip(vwap_score_up, 0.0, 1.0)) + 0.20 * float(vwap_reclaim_hold) - 0.12 * float(vwap_reclaim_cross and not vwap_reclaim_hold))
        if orb_high is not None and np.isfinite(orb_high):
            accept_components["ORB"] = float(orb_high)
            accept_component_weights["ORB"] = max(0.05, 0.28 + 0.55 * float(np.clip(orb_score_up, 0.0, 1.0)) + 0.18 * float(orb_accept_up) + 0.14 * float(orb_retest_up) - 0.14 * float(orb_break_up and not orb_accept_up))
        if swing_hi is not None and np.isfinite(swing_hi):
            accept_components["PIVOT"] = float(swing_hi)
            accept_component_weights["PIVOT"] = max(0.05, 0.24 + 0.55 * float(np.clip(pivot_score_up, 0.0, 1.0)) + 0.18 * float(pivot_hold_up) - 0.12 * float(pivot_break_up and not pivot_hold_up))
        if ema20_ref is not None and np.isfinite(ema20_ref):
            accept_components["EMA20"] = float(ema20_ref)
            accept_component_weights["EMA20"] = 0.10 + 0.08 * float(trend_votes >= 2)
    else:
        if ref_vwap is not None and np.isfinite(ref_vwap):
            accept_components["VWAP"] = float(ref_vwap)
            accept_component_weights["VWAP"] = max(0.05, 0.30 + 0.55 * float(np.clip(vwap_score_dn, 0.0, 1.0)) + 0.20 * float(vwap_reject_hold) - 0.12 * float(vwap_reject_cross and not vwap_reject_hold))
        if orb_low is not None and np.isfinite(orb_low):
            accept_components["ORB"] = float(orb_low)
            accept_component_weights["ORB"] = max(0.05, 0.28 + 0.55 * float(np.clip(orb_score_dn, 0.0, 1.0)) + 0.18 * float(orb_accept_dn) + 0.14 * float(orb_retest_dn) - 0.14 * float(orb_break_dn and not orb_accept_dn))
        if swing_lo is not None and np.isfinite(swing_lo):
            accept_components["PIVOT"] = float(swing_lo)
            accept_component_weights["PIVOT"] = max(0.05, 0.24 + 0.55 * float(np.clip(pivot_score_dn, 0.0, 1.0)) + 0.18 * float(pivot_hold_dn) - 0.12 * float(pivot_break_dn and not pivot_hold_dn))
        if ema20_ref is not None and np.isfinite(ema20_ref):
            accept_components["EMA20"] = float(ema20_ref)
            accept_component_weights["EMA20"] = 0.10 + 0.08 * float(trend_votes >= 2)
    ...
    def _build_ride_entry_geometry(conviction_score: float) -> dict[str, object]:
        ...
        if breakout_ref_local is not None and len(recent_break_closes_local):
            if direction == "LONG":
                breakout_acceptance_ok_local = bool((recent_break_closes_local >= float(breakout_ref_local) - buffer).any() and float(recent_break_closes_local.iloc[-1]) >= float(breakout_ref_local) - buffer)
            else:
                breakout_acceptance_ok_local = bool((recent_break_closes_local <= float(breakout_ref_local) + buffer).any() and float(recent_break_closes_local.iloc[-1]) <= float(breakout_ref_local) + buffer)
        else:
        ...
        rejection_ctx_local = {'penalty': 0.0, 'stuffing': False, 'wick_ratio': 0.0, 'close_finish': 0.0}
        breakout_urgency_local = 0.0
        if actionable_local:
            ...
            breakout_elite_pre = bool(stage == "PRE" and (impulse_type_hint in ("ORB", "VWAP")) and impulse_legitimacy >= 0.86 and accept_ok and breakout_acceptance_ok_local and breakout_extension_ok)
            breakout_confirmed_ok = bool(stage == "CONFIRMED" and (impulse_type_hint in ("ORB", "VWAP")) and impulse_legitimacy >= 0.72 and breakout_acceptance_ok_local and breakout_extension_ok)
            ...
            if tape_mode_enabled:
                tape_metrics_local = _compute_tape_readiness(
                    ...
                    trigger_near=bool(near_break_local or (dist_br_local <= 1.15 * prox_dist_local)),
                    baseline_ok=bool(
                        (impulse_quality >= 0.48)
                        and (impulse_legitimacy >= 0.55)
                        and (accept_ok or hold_ok or breakout_acceptance_ok_local)
                        and (not stale_breakout_local)
                    ),
                )
                tape_score_bonus_local = _tape_bonus_from_readiness(float(tape_metrics_local.get("readiness") or 0.0), cap=5)
                rejection_ctx_local = _compute_release_rejection_penalty(
                    df, direction=str(direction), atr_last=float(atr_last) if atr_last is not None else None, release_level=float(break_trigger_local),
                )
                breakout_urgency_local = _compute_breakout_urgency(
                    tape_metrics=tape_metrics_local,
                    near_break=bool(near_break_local),
                    dist_br=float(dist_br_local),
                    dist_pb_band=float(dist_pb_band_local),
                    prox_dist=float(prox_dist_local),
                    hold_progress=bool(hold_progress_local),
                    shallow_retests=bool(shallow_retests_local),
                    breakout_acceptance_ok=bool(breakout_acceptance_ok_local),
                    rejection_penalty=float(rejection_ctx_local.get('penalty') or 0.0),
                )
                tape_breakout_ready_local = bool(
                    float(tape_metrics_local.get("readiness") or 0.0) >= 6.0
                    and float(tape_metrics_local.get("pressure") or 0.0) >= 1.5
                    and bool(tape_metrics_local.get("macd_directional_build") or False)
                    ...
                    and not stale_breakout_local
                    and dist_br_local <= min(dist_pb_band_local, 0.75 * prox_dist_local)
                    and float(rejection_ctx_local.get('penalty') or 0.0) < 1.0
                    and breakout_urgency_local >= 3.5
                )
                if tape_breakout_ready_local and breakout_urgency_local >= 4.25:
                ...
                tape_prefers_breakout_local = bool(
                    ...
                    and float(tape_metrics_local.get("release_proximity") or 0.0) >= 1.5
                    and dist_br_local <= min(dist_pb_band_local, 0.65 * prox_dist_local)
                    and breakout_urgency_local >= 4.5
                    and float(rejection_ctx_local.get('penalty') or 0.0) <= 0.5
                )
                if float(rejection_ctx_local.get('penalty') or 0.0) >= 0.5 and breakout_bias_score_local > 0:
                    breakout_bias_score_local -= 1
            ...
            if not breakout_bias:
                breakout_bias = bool(near_break_local and not stale_breakout_local and breakout_bias_score_local >= (5 if stage == "PRE" else 4))
            if not breakout_bias and tape_prefers_breakout_local:
                breakout_bias = True
            ...
            if near_pullback_local and not near_break_local:
                ...
                choose_pullback = bool(near_pullback_local)
        ...
        return {
            ...
            "tape_rejection_penalty": float(rejection_ctx_local.get('penalty') or 0.0) if isinstance(rejection_ctx_local, dict) else 0.0,
            "tape_stuffing": bool(rejection_ctx_local.get('stuffing') or False) if isinstance(rejection_ctx_local, dict) else False,
            "tape_breakout_urgency": float(breakout_urgency_local) if 'breakout_urgency_local' in locals() else 0.0,
        }
    ...
    return SignalResult(
        ...
        extras={
            ...
            "tape_rejection_penalty": float(final_geometry.get("tape_rejection_penalty") or 0.0),
            "tape_stuffing": bool(final_geometry.get("tape_stuffing") or False),
            "tape_breakout_urgency": float(final_geometry.get("tape_breakout_urgency") or 0.0),
